...
func research_has_found_something(delta):
	if (enemyController.isInResearch && guardResearch.researchLaunched && !guardResearch.researchHasFoundSomething && guardResearch.stunnedGuardsList.size() == 0 && guardResearch.suspiciousItemsList.size() == 0):
